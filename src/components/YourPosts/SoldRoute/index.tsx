...
const SoldRoute: React.FC<SoldRouteProps> = ({navigation}) => {
  ...
  const getPersonalPost = async () => {
    ...
    let soldPostListTmp: Array<personalPostInfoType> = [];
    for (let i = 0; i < postListID.length; i++) {
      soldPostListTmp.push(await getPostDetailPersonal(postListID[i]));
    }
    setSoldPostList(soldPostListTmp);
    setIsLoading(false);
    if (soldPostListTmp.length == 0) setIsEmpty(true);
  };
